...
if st.button('Submit'):
    ipfs_hash, token_json = pin_artwork(crowdfund_name, file)
    uri = f'ipfs://{ipfs_hash}'
    print(crowdfund_name, description, uri)
    submission = contract.functions.createToken(crowdfund_name, description, beneficiary_address, uri).transact({'from': creator_address, 'gas': 1000000})
