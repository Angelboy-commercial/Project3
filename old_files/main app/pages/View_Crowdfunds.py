...
st.title("Donate")
address = st.selectbox("Donor Account", options=accounts)
token_id = st.text_input("Token")
donation = st.text_input("Amount")
...
if st.button("End Auction"):
    end = tx_hash = contract.functions.end(int(token_id)).transact({"from" : address, 'gas' : 3000000})
    receipt = w3.eth.waitForTransactionReceipt(end)
    st.write("Transaction receipt mined:")
    st.write(dict(receipt))
