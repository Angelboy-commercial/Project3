...
import "http://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v3.0.0/contracts/token/ERC721/ERC721.sol";
...
contract Auctions is ERC721 {
    ...
    function createToken(string memory name, string memory description, address beneficiary, uint auctionEndTime, uint startingBid, string memory tokenURI) public {
        ...
        tokenCollection[tokenId] = Token(msg.sender, name, description, beneficiary, false, auctionEndTime, startingBid, 0, msg.sender);
    }
    ...
    function withdraw () public ends(tokenId) {
    ...
    function viewToken() public view returns(address, string memory, string memory, address, bool, uint, uint, uint, address) {
        Token storage token = tokenCollection[tokenId];

        return (token.creator, token.name, token.description, token.beneficiary, token.auctionComplete, token.auctionEndTime, token.startingBid, token.highestBid, token.highestBidder);
    }
