...
class Registration:
    def __init__(self, contract, w3, accounts):
    ...
    def nft_registration(self):
        ...
        address = st.selectbox("Select Account", options=self.accounts)
        st.markdown("---")

        st.markdown("## Registration")
        name = st.text_input("Enter the name of the NFT")
        ...
        if st.button("Register NFT"):
            ...
            print(name, artist, description, starting_bid, uri)
            tx_hash = self.contract.functions.registerArtwork(
            address,
            name,
            artist,
            description,
